...
def process_patent_text(
    patent_text: str,
    associated_molecules: list[dict],
    patent_id: str,
    config: ConfigLLM,
    debug: bool = False,
    ...
    logging.info(
        f"Found {len(relevant_chunks)} relevant chunks for patent {patent_id}."
    )
    ...
    for chunk in tqdm(relevant_chunks, desc=f"Processing chunks for {patent_id}"):
        raw_mentions, extracted_data = agent.run(
            chunk, patent_id, debug, debug_output_dir
        )
        if extracted_data:
            ...
            for item in extracted_data:
                ...
                if isinstance(item, dict):
                    if (
                        item.get("value")
                        and item.get("molecule_name")
                        and item.get("binding_metric")
                        and item.get("protein_target_name")
                    ):
                        item[
                            "raw_mentions"
                        ] = raw_mentions  # Add the source mentions for debugging
                        all_extracted_data.append(item)

    logging.info(
        f"Extracted {len(all_extracted_data)} data points from patent {patent_id}."
    )
